...
procedure TFormTambahLokasi.Button1Click(Sender: TObject);
  begin
    if (edit1.Text <> '') and (edit2.Text <> '') and (edit3.Text <> '') and (ComboBox1.ItemIndex >= 0) and (ComboBox2.ItemIndex >= 0) and (ComboBox3.ItemIndex >= 0) then
      begin
      try
        DataModule1.ZQueryTambahLokasi.SQL.Text := 'INSERT INTO lokasi (nama_tempat, koordinat, alamat, id_desa, id_kecamatan, id_jenis_tempat_ibadah)' + 'VALUES (:nama_tempat, :koordinat, :alamat, :id_desa, :id_kecamatan, :id_jenis_tempat_ibadah)';
        ...
        DataModule1.ZQueryTambahLokasi.SQL.Text := 'SELECT lokasi.id_lokasi, lokasi.nama_tempat, lokasi.koordinat, lokasi.alamat, desa.desa, kecamatan.kecamatan, jenis_tempat_ibadah.jenis_tempat_ibadah' +
        ' FROM lokasi'+
        ' INNER JOIN desa ON lokasi.id_desa=desa.id_desa' +
        ' INNER JOIN kecamatan ON lokasi.id_kecamatan=kecamatan.id_kecamatan' +
        ' INNER JOIN jenis_tempat_ibadah ON lokasi.id_jenis_tempat_ibadah=jenis_tempat_ibadah.id_jenis_tempat_ibadah';
...
if (edit1.Text <> '') and (edit2.Text <> '') and (edit3.Text <> '') and (ComboBox1.ItemIndex >= 0) and (ComboBox2.ItemIndex >= 0) and (ComboBox3.ItemIndex >= 0) then
      ...
      try
        ...
        DataModule1.ZQueryTambahLokasi.SQL.Text := 'UPDATE lokasi SET nama_tempat = :nama_tempat, koordinat = :koordinat, alamat = :alamat, id_desa = :id_desa, id_kecamatan = :id_kecamatan, id_jenis_tempat_ibadah = :id_jenis_tempat_ibadah WHERE id_lokasi = :id_lokasi';
        DataModule1.ZQueryTambahLokasi.Params.ParamByName('nama_tempat').Value := Edit1.Text;
        DataModule1.ZQueryTambahLokasi.Params.ParamByName('koordinat').Value := Edit2.Text;
        DataModule1.ZQueryTambahLokasi.Params.ParamByName('alamat').Value := Edit3.Text;
        DataModule1.ZQueryTambahLokasi.Params.ParamByName('id_desa').Value := Integer(ComboBox1.Items.Objects[ComboBox1.ItemIndex]);
...
begin
  ...
  begin
    ...
    begin
    DataModule1.ZQueryTambahLokasi.SQL.Text := 'DELETE FROM lokasi WHERE id_lokasi = :id_lokasi';
    DataModule1.ZQueryTambahLokasi.Params.ParamByName('id_lokasi').Value := IDLokasi;
    DataModule1.ZQueryTambahLokasi.ExecSQL;

    DataModule1.ZQueryTambahLokasi.SQL.Text := 'ALTER TABLE lokasi AUTO_INCREMENT = 1';
    DataModule1.ZQueryTambahLokasi.ExecSQL;
...
begin
  ...
  DataModule1.ZQueryTambahDesa.Active := False;
  ...
  DataModule1.ZQueryTambahKecamatan.Active := False;
  ...
  DataModule1.ZQueryTambahJenisTempatIbadah.Active := False;
end;
